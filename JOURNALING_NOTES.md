...
>
> _"≥1 photo per attraction" pass DONE (added 6 `.mag-spread` pages, all Chromium-verified non-clip, using photos already in repo — no dups): Kohl Fountain → detail `11` ("The Fountain, Up Close"); St George's → nave `24` ("The Nave of St. George's"); Old Royal Palace → heraldic ceiling `29` ("The Heraldic Hall"); Throne → Crown Jewels `33` ("The Crown Jewels", seven-locks story); Týn → towers close-up `21` ("The Týn Towers"); St Nicholas → exterior `19` ("St. Nicholas, Outside"). Issue now ~33 pages. **STILL OPEN:** the "Tastes of Bohemia" collage page overflows ~208px in headless render (pre-existing, not fixed — needs a look). Astronomical Clock still has just 1 photo (day-05/12); an all-5 family selfie at the clock is in this session's transcript, not yet processed. Held-but-unused: `25/26` (St George apse/relief), `28` (palace heraldic wall), `30` (castle model), `31` (freed rose-window)._
